...
def getTree(itop_params, ci_type):
    ...
    try:
        ...
        response = requests.get(url, auth=(itop_params["user"],
                                           itop_params["password"]))
        ...
        sys.exit("ERROR: Can't get XML export from iTop.")


def main():
    # Script arguments
    description = "Synchronize iTop CMDB CIs to LogicMonitor"
    ...
    # As of iTop 2.2, there is no "managementip" property for Printer or
    # Hypervisor CIs, so only Server is synchronized in this script.
